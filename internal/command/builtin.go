...
func (b *Builtins) Certs(cmd Command) Response {
	...
	script := `for d in /etc/letsencrypt/live/*/cert.pem; do domain=$(basename $(dirname "$d")); expiry=$(openssl x509 -enddate -noout -in "$d" 2>/dev/null | cut -d= -f2); echo "$domain: $expiry"; done`
...
func (b *Builtins) Traffic(cmd Command) Response {
	...
	if cmd.Target != "" {
		...
		// Filter logs for this domain and pipe to rhit
		script := fmt.Sprintf(
			"sudo zcat -f /var/log/nginx/access.log* | grep -i '%s' | rhit - --color no -d '%s' --length 10 2>&1",
			domain, dateRange)
		result, err := b.exec.Run(ctx, "bash", "-c", script)
		if err != nil {
	...
	result, err := b.exec.Run(ctx, "sudo", "rhit", "/var/log/nginx/", "--color", "no", "-d", dateRange, "--length", "15")
	if err != nil {
		return errorResponse("Traffic Failed", err.Error())
	}
	return Response{
...
func (b *Builtins) Analytics(cmd Command) Response {
	...
	// GoAccess CSV output, parse into readable summary
	result, err := b.exec.Run(ctx, "bash", "-c",
		"sudo goaccess /var/log/nginx/access.log --log-format=COMBINED --no-term-resolver --no-color --output-format csv -o - 2>/dev/null | head -20")
	if err != nil {
	...
	var sb strings.Builder
	sb.WriteString("📊 Web Analytics (today)\n\n")
	for _, line := range strings.Split(result.Output, "\n") {
		parts := strings.Split(line, ",")
		if len(parts) >= 12 {
			key := strings.Trim(parts[11], "\"")
			val := strings.Trim(parts[10], "\"")
			switch key {
			case "total_requests":
			...
			case "unique_files":
				sb.WriteString(fmt.Sprintf("📄 Unique pages: %s\n", val))
			case "bandwidth":
				n, _ := fmt.Sscanf(val, "%d")
				if n > 0 {
					sb.WriteString(fmt.Sprintf("📡 Bandwidth: %s bytes\n", val))
				}
			case "unique_not_found":
				sb.WriteString(fmt.Sprintf("❌ 404 errors: %s\n", val))
			case "unique_referrers":
				sb.WriteString(fmt.Sprintf("🔗 Referrers: %s\n", val))
			case "date_time":
